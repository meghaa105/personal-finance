...
export default function ThemeToggle() {
    ...
    useEffect(() => {
        ...
        gsap.set(cloudRef.current, { x: 75 });
        gsap.set([sunRef.current, moonRef.current], { x: 50 });
        gsap.set(starRefs.current, { x: 35, y: -5 });
    }, []);

    const handleDayClick = () => {
        setIsDay(false);
        toggleTheme();
        gsap.to(sunRef.current, { duration: 1, x: -157, opacity: 0, ease: 'power1.easeInOut' });
        gsap.to(cloudRef.current, { duration: 0.5, opacity: 0, ease: 'power1.easeInOut' });
        gsap.to(moonRef.current, { duration: 1, x: -197, rotate: -360, transformOrigin: 'center', opacity: 1, ease: 'power1.easeInOut' });
        gsap.to(starRefs.current, { duration: 0.5, opacity: 1, ease: 'power1.easeInOut' });
        gsap.to(nightRef.current, { duration: 1, background: '#224f6d', borderColor: '#cad4d8', ease: 'power1.easeInOut' });
        gsap.to(backgroundRef.current, { duration: 1, background: '#0d1f2b', ease: 'power1.easeInOut' });
    };

    const handleNightClick = () => {
        setIsDay(true);
        toggleTheme();
        gsap.to(sunRef.current, { duration: 1, x: 50, opacity: 1, ease: 'power1.easeInOut' });
        gsap.to(cloudRef.current, { duration: 1, x: 75, opacity: 1, ease: 'power1.easeInOut' });
        gsap.to(moonRef.current, { duration: 1, opacity: 0, x: 35, rotate: 360, transformOrigin: 'center', ease: 'power1.easeInOut' });
        gsap.to(starRefs.current, { duration: 0.5, opacity: 0, ease: 'power1.easeInOut' });
        gsap.to(nightRef.current, { duration: 1, background: '#9cd6ef', borderColor: '#65c0e7', ease: 'power1.easeInOut' });
        gsap.to(backgroundRef.current, { duration: 1, background: 'white', ease: 'power1.easeInOut' });
    };

    return (
        <div className='flex gap-2 items-center'>
            <div className='themeToggle relative h-10 cursor-pointer'>
                <div
                    ref={dayRef}
                    id="day"
                    onClick={isDay ? handleDayClick : handleNightClick}
                    className={`bg-sky-400 w-24 h-10 absolute ${isDay ? 'z-[1]' : 'z-[0]'} pointer rounded-[6.25rem] border-4 border-gray-600`}
                ></div>
                <div
                    ref={nightRef}
                    id="night"
                    onClick={!isDay ? handleNightClick : handleDayClick}
                    className={`bg-sky-400 w-24 h-10 absolute ${!isDay ? 'z-[1]' : 'z-[0]'} pointer rounded-[6.25rem] border-4 border-gray-600`}
                ></div>
                <svg
                    version="1.1"
                    ...
                    xmlSpace="preserve"
                    className={`scale-90 w-24 h-10 z-[2] pointer-events-none absolute top-0`}
                >
                    <g className='w-32'>
                        <polygon className="star" ref={(el) => (starRefs.current[0] = el)} id="star-1" fill="#A2B5BF" points="166.253,132.982 164.364,135.676 160.983,136.488 163.196,138.965 162.996,142.16
                        166.253,140.998 169.509,142.16 169.309,138.965 171.522,136.488 168.142,135.676 	"/>
                        <polygon className="star" ref={(el) => (starRefs.current[1] = el)} id="star-2" fill="#A2B5BF" points="175.522,44.243 172.684,48.29 167.603,49.51 170.929,53.233 170.628,58.035
                        175.522,56.288 180.417,58.035 180.116,53.233 183.442,49.51 178.361,48.29 	"/>
                        <polygon className="star" ref={(el) => (starRefs.current[2] = el)} id="star-3" fill="#A2B5BF" points="208.22,91.845 206.083,94.891 202.259,95.81 204.763,98.61 204.535,102.226
                        208.22,100.911 211.903,102.226 211.677,98.61 214.181,95.81 210.356,94.891 	"/>
                        <polygon className="star" ref={(el) => (starRefs.current[3] = el)} id="star-4" fill="#A2B5BF" points="252.545,39.052 250.409,42.098 246.585,43.017 249.089,45.819 248.86,49.433
                        252.545,48.118 256.229,49.433 256.002,45.819 258.506,43.017 254.682,42.098 	"/>
                        <polygon className="star" ref={(el) => (starRefs.current[4] = el)} id="star-5" fill="#A2B5BF" points="280.151,84.949 282.749,88.997 287.401,90.217 284.355,93.94 284.632,98.742
                        280.151,96.995 275.669,98.742 275.946,93.94 272.899,90.217 277.552,88.997 	"/>
                        <polygon className="star" ref={(el) => (starRefs.current[5] = el)} id="star-6" fill="#A2B5BF" points="249.791,124.466 246.668,128.919 241.076,130.261 244.737,134.356 244.405,139.64
                    ...
                    <g className='w-32'>
                        ...
                        <path ref={cloudRef} id="cloud" fill="#ECF0F1" stroke="#CAD4D8" strokeWidth="6" strokeMiterlimit="10" d="M153.269,109.614h2.813
            ...
            <span className='text-xl font-bold text-slate-700 dark:text-slate-400 translate-x-24'>{isDay ? "Light" : "Dark"} Mode</span>
        </div>
    )
}