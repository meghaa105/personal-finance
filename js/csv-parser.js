...
const CSVParser = (function() {
    ...
    function guessCategory(description) {
        ...
        if (descriptionLower.startsWith('upi/')) {
            console.log('UPI transaction detected');
            const upiParts = description.split('/');
            if (upiParts.length >= 5) {
                const relevantFields = `${upiParts[3]} ${upiParts[4]}`.trim();
                console.log('Extracted UPI fields for categorization:', relevantFields);
                return categorizeDescription(relevantFields);
            }
        }
        ...
        return categorizeDescription(descriptionLower);
    }
    function categorizeDescription(description) {
        ...
        const categoryPatterns = {
            ...
            'Transportation': [/(?:uber|ola|rapido|metro|bus|train|taxi|auto|petrol|diesel|fuel|fastag|parking|toll|fare|railway|irctc|redbus|ticket|travel.*transport|cab|rickshaw|auto|TSRTC)/i],
            ...
            'Health': [/(?:hospital|clinic|medical|pharmacy|medicine|doctor|apollo|fortis|diagnostic|lab|test|wellness|dental|health|treatment|consultation|physician|specialist|medplus|netmeds|1mg|pharmeasy|pullela)/i],
            ...
            'Entertainment': [/(?:movie|cinema|pvr|inox|netflix|prime|hotstar|entertainment|game|gaming|theatr|show|concert|event|ticket.*show|sport|recreation|amusement|fun|leisure|subscription|streaming|party)/i],
            ...
            'Investments': [/(?:mutual\s*fund|stock|share|demat|investment|zerodha|indmoney|indianclearingcorpora|sip|nps|ppf|trading|portfolio|wealth|asset|equity|bond|etf|gold|deposit|fd|rd)/i],
